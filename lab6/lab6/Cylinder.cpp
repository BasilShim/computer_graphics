...
std::vector<glm::vec3> Cylinder::getPoints(bool side)
{
    ...
    for (int i = 1; i < numberOfVertices; i++)
    {
        ...
        temp.y = center.y;
        temp.z = center.z + (2.0f * radius * sin(i * doublePi / numberOfSides));
...
std::vector<GLfloat> Cylinder::getIndices()
{
    ...
    {
        glm::vec3 normal = glm::triangleNormal(centerUp, pointsUp[i], pointsUp[(i + 1) % pointsUp.size()]);
    ...
    {
        glm::vec3 normal = glm::triangleNormal(centerBottom, pointsBottom[i], pointsBottom[(i + 1) % pointsBottom.size()]);
    ...
    for (int i = 0; i < pointsBottom.size(); i++)
    {
        glm::vec3 normal = glm::triangleNormal(pointsUp[i], pointsBottom[i], pointsUp[(i + 1) % pointsUp.size()]);
